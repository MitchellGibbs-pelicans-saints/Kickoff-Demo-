...
function IdeaIntake({ state, updateState }: { state: DemoState; updateState: (updater: (current: DemoState) => DemoState) => void }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    updateState((current) => ({ ...current, proposals: [proposal, ...current.proposals], audit: [{ id: nextId(current.audit.map((event) => event.id), 'a'), actor: user.name, action: 'Created proposal draft', target: title, at: '2026-08-07 10:42' }, ...current.audit] }))
...
function ApprovalsPage({ user, state, updateState }: { user: User; state: DemoState; updateState: (updater: (current: DemoState) => DemoState) => void }) {
  ...
  const decide = (proposal: Proposal, status: ProposalStatus) => updateState((current) => ({ ...current, proposals: current.proposals.map((item) => item.id === proposal.id ? { ...item, status, approvalStages: recordDecision(stagesFor(item), status, user.name), updatedAt: '2026-08-07', version: item.version + 1 } : item), audit: [{ id: nextId(current.audit.map((event) => event.id), 'a'), actor: user.name, action: status === 'Approved' ? 'Approved proposal' : status === 'Rejected' ? 'Rejected proposal' : 'Requested changes', target: proposal.title, at: '2026-08-07 10:42' }, ...current.audit] }))
...
function AdminUsers({ user, state, updateState }: { user: User; state: DemoState; updateState: (updater: (current: DemoState) => DemoState) => void }) {
  ...
  const toggleActive = (target: User) => updateState((current) => ({ ...current, users: current.users.map((item) => item.id === target.id ? { ...item, active: !item.active } : item), audit: [{ id: nextId(current.audit.map((event) => event.id), 'a'), actor: user.name, action: target.active ? 'Revoked demo access' : 'Restored demo access', target: target.name, at: '2026-08-07 10:42' }, ...current.audit] }))
  const toggleRole = (target: User, role: Role) => updateState((current) => ({ ...current, users: current.users.map((item) => item.id === target.id ? { ...item, roles: item.roles.includes(role) ? item.roles.filter((itemRole) => itemRole !== role) : [...item.roles, role] } : item), audit: [{ id: nextId(current.audit.map((event) => event.id), 'a'), actor: user.name, action: target.roles.includes(role) ? `Removed ${role} role` : `Assigned ${role} role`, target: target.name, at: '2026-08-07 10:42' }, ...current.audit] }))
