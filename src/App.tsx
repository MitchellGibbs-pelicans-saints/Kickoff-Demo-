...
function ProposalPage({ state, user }: { state: DemoState; user: User }) {
  ...
  return <div className="view-enter">
    ...
    {tab === 'Status & feedback' && <StatusFeedback proposal={proposal} user={user} users={state.users} />}
...
function StatusFeedback({ proposal, user, users }: { proposal: Proposal; user: User; users: User[] }) {
  const feedback = visibleFeedback(user, proposal)
...
function AdminDepartments({ user, state, updateState }: { user: User; state: DemoState; updateState: (updater: (current: DemoState) => DemoState) => void }) {
  ...
  const changeThreshold = (field: 'confidenceThreshold' | 'collaboratorThreshold', value: number) => writeConfig(`${field} changed to ${value}.`, (config) => ({ ...config, [field]: value }))
